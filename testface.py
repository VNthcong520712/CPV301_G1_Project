				...
				for img_file in files:
					if img_file.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
						...
						try:
							...
							# Phát hiện vị trí khuôn mặt. Có thể dùng model="cnn" nếu có GPU.
    ...
    def run(self):
        while not self.stop_event.is_set():
            try:
                ...
                print(f"Thời gian lấy vị trí khuôn mặt: {end_face_detection_time - start_face_detection_time:.4f} giây")
                        ...
                        if min_distances:
                            ...
                            if min_distance < 0.6:
                                ...
                                all(name == match_name for name in self.detect_name_buffer):
                                    ...
                                    if self.last_confirmed_name != detected_name:
...
class AttendanceGUI:
	...
	def _update_frame_display(self):
		...
		"""
		if self.processing_thread.frame_to_display is not None:
